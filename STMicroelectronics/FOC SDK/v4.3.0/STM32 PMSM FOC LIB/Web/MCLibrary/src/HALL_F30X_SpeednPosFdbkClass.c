...
static void HALL_IRQHandler(void *this, unsigned char flag)
{
  ...
  {    
    ...
    else if ((bNewSpeed * pDVars_str->bSpeed) > 0)
    {      
  ...
  if (pDVars_str->bNewSpeedAcquisition == 1)
  {
    ...
    else
    {
      ...
      for (bIndex = 0u; bIndex < bSpeedBufferSize; bIndex++ )
      {
        ...
        wPeriod = (int32_t)pDVars_str->SensorPeriod[bIndex].wPeriod;
        bDirection = pDVars_str->SensorPeriod[bIndex].bDirection;
        
        TIMx->DIER |= TIM_IT_CC1;   /* NB:Std libray not used for perf issue*/
                
        wElSpeedSum += ((int32_t)(pDVars_str->wPseudoFreqConv)/wPeriod) * bDirection;
      }
      
      hAvrgElSpeed = (int16_t)(wElSpeedSum / (int32_t)(bSpeedBufferSize));        /* Average value */
    }

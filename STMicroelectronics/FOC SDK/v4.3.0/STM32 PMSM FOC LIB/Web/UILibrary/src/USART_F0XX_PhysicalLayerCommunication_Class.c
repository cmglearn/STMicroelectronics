...
static uint8_t USART_AF(USART_TypeDef* USARTx, GPIO_TypeDef* GPIOx);
...
void USART_HWInit(pUSARTParams_t pUSARTParams)
{
  ...
  GPIO_PinAFConfig(pUSARTParams->hRxPort, USART_GPIOPin2Source(pUSARTParams->hRxPin), USART_AF(pUSARTParams->USARTx, pUSARTParams->hRxPort));
  GPIO_PinAFConfig(pUSARTParams->hTxPort, USART_GPIOPin2Source(pUSARTParams->hTxPin), USART_AF(pUSARTParams->USARTx, pUSARTParams->hRxPort));
...
void* USART_IRQ_Handler(void* this,unsigned char flags, unsigned short rx_data)
{
  ...
  if (flags == 0) // Flag 0 = RX
  {
    ...
    {
      rx_data = ((_CUSART)(((_CCOM)this)->DerivedClass))->pDParams_str->USARTx->RDR;
